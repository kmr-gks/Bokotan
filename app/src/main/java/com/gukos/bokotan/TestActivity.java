...
public class TestActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		checkBoxHatsuon.setChecked(getSharedPreferences("checkBoxHatsuon", MODE_PRIVATE).getBoolean("checked", true));
		checkBoxHatsuon.setOnClickListener(view -> getSharedPreferences("checkBoxHatsuon", MODE_PRIVATE).edit().putBoolean("checked", ((CheckBox) view).isChecked()).apply());
		checkBoxKoukaon.setChecked(getSharedPreferences("checkBoxKoukaon", MODE_PRIVATE).getBoolean("checked", true));
		checkBoxKoukaon.setOnClickListener(view -> getSharedPreferences("checkBoxKoukaon", MODE_PRIVATE).edit().putBoolean("checked", ((CheckBox) view).isChecked()).apply());
		...
		setMondaiBun();
		sp.setOnLoadCompleteListener((soundPool, i, i1) -> soundPool.play(i, 1f, 1f, 1, 0, 1f));
	}
	...
	public void setMondaiBun(){
		...
		puts("f"+nRangeForOptionsFrom+"t"+nRangeForOptionsTo);
		do {
		...
		if (checkBoxHatsuon.isChecked()){
			...
			if (strQ.startsWith("y")){
				nSound=sp.load("/storage/emulated/0/Download/data/" + Q_sentaku_activity.strQenum.getQ + '/' + String.format("W英%04d", nMondaiTangoNum)  + ".mp3",1);
			}else{
